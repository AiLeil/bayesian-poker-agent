...
### 核心亮点
* **小样本适应性：** 利用 `Beta Distribution` 解决早期对手数据不足的冷启动问题。
* **纯 Python 引擎：** 纯手写 Python 扑克物理引擎，模拟真实牌局状态机，不依赖黑盒库。
* **量化思维：** 引入凯利公式进行资金管理，并包含盈亏回测分析模块。
...
```bash
# 1. 克隆仓库
git clone https://github.com/AiLeil/bayesian-poker-agent.git
